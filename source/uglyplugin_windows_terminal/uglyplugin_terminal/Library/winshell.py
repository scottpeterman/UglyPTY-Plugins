...
from uglyplugin_terminal.Library.winshellreader import ShellReaderThread
...
class Backend(QObject):
    ...
    def __init__(self, shell="cmd.exe", parent=None):
        ...
        if platform.system() == "Windows":
            if ".virtualenvs" in str(sys.executable):
                # pipenv helper
                if "wsl" in shell:
                    self.process = PtyProcess.spawn([shell])
                else:
                    self.process = PtyProcess.spawn([shell, "/c", "pipenv", "shell"])
            else:
                self.process = PtyProcess.spawn([shell])
