...
query_default = """SELECT * FROM df WHERE display_name like '%router%'"""
...
def fetch_devices(yaml_file: str, query=None) -> List[Dict]:
    try:
        ...
        print(f"fetch_devices failed", f"Failure: {e}")
        notify("fetch_devices failed", f"Failure: {e}")
        return []
